function [corners, nMatches, avgErr, intersections] = findCheckerBoard(Io)
...
%     thetaValues(peaks(:,2)), ... % thetas fomat = [p1(1)-p3(1) p1(2)-p3(2); ...
...
% form [ [x1,y1]; [x2,y2]; ... ].
[corners, nMatches, avgErr, intersections] = findCorrespondence( ...
...
% outer corner points from the image, represented as a a 4x2 array, in the
% form [ [x1,y1]; [x2,y2], ... ].
function [corners, nMatchesBest, avgErrBest, intersectsBest] = findCorrespondence( ...
    xIntersections, yIntersections, ... % Input image points
    xIntersectionsRef, yIntersectionsRef, ... % Reference image points
    sizeImg, I)
    ...
    for i1b=N1:-1:max(N1-M,i1a+1)
        for i2a=1:min(M,N2)
            for i2b=N2:-1:max(N2-M,i2a+1)
                ...
                corners = pCornersImg;
                intersectsBest = pIntersectionsRefWarp;
%                 % Display the predicted and measured points.
%                 figure(14), imshow(zeros(sizeImg));
